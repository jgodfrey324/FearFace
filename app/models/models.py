...
class ProductImage(db.Model):
    ...
    product = db.relationship(
        'Product', back_populates='product_images')  # double check

    def __repr__(self):
        return f'<Image {self.id} is associated with Product {self.product.id}, {self.product.name}>'

    # think about how we can get product_images into the product for the slice of state..
    def to_dict(self):
        return {
            'id': self.id,
            'url': self.url,
...
class PostImage(db.Model):
    ...
    post = db.relationship(
        'Post', back_populates='post_images')  # double check

    def __repr__(self):
        return f'<Image {self.id} is associated with Post {self.post_id}>'

    # think about how we can get post_images into the posts for the slice of state..
    def to_dict(self):
        return {
            'id': self.id,
            'url': self.url,
